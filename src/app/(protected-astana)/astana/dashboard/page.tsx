...
export default function AstanaDashboardPage() {
  ...
  const translations = {
    EN: {
      ...
      stepDescActive: 'Your registration is being reviewed by the Astana office.',
      ...
      legalForm: 'Legal Form (e.g. Private Company Limited by Shares)',
      shareCapital: 'Authorized Share Capital (USD)',
      shareholders: 'No. of Shareholders',
      directors: 'No. of Directors',
      personData: '4. SHAREHOLDER / DIRECTOR DETAILS',
      fullName: 'Full Name (as in passport)',
      position: 'Position (Shareholder / Director)',
      ...
      proofAddress: 'Proof of Residential Address',
      proofAddressDesc: 'Utility bill or bank statement, last 3 months',
      cv: 'CV / Resume',
      cvDesc: 'Current professional summary',
      bankRef: 'Bank Reference Letter',
      bankRefDesc: 'If applicable',
    ...
    RU: {
      ...
      stepDescActive: 'Ваша заявка рассматривается офисом в Астане.',
      ...
      legalForm: 'Организационно-правовая форма (напр. Private Company Limited by Shares)',
      shareCapital: 'Уставный капитал (USD)',
      shareholders: 'Кол-во акционеров',
      directors: 'Кол-во директоров',
      personData: '4. ДАННЫЕ АКЦИОНЕРА / ДИРЕКТОРА',
      fullName: 'ФИО (как в паспорте)',
      position: 'Должность (Акционер / Директор)',
      ...
      proofAddress: 'Подтверждение адреса проживания',
      proofAddressDesc: 'Счёт за коммунальные услуги или банковская выписка за последние 3 месяца',
      cv: 'Резюме (CV)',
      cvDesc: 'Актуальное профессиональное резюме',
      bankRef: 'Банковское рекомендательное письмо',
      bankRefDesc: 'Если применимо',
    ...
    CHI: {
      ...
      currentStep: '我的申请',
      stepDesc: '没有有效的申请',
      stepDescActive: '阿斯塔纳办事处正在审核您的注册。',
      ...
      structure: '3. 公司结构',
      legalForm: '法律形式（例如 Private Company Limited by Shares）',
      shareCapital: '注册股本（美元）',
      shareholders: '股东人数',
      directors: '董事人数',
      personData: '4. 股东/董事详细信息',
      fullName: '全名（与护照一致）',
      position: '职位（股东/董事）',
      passport: '护照（系列，号码）',
      contactNumber: '联系电话',
      ...
      proofAddress: '居住地址证明',
      proofAddressDesc: '最近3个月的水电费账单或银行对账单',
      cv: '个人简历',
      cvDesc: '最新的专业简历',
      bankRef: '银行推荐信',
      bankRefDesc: '如适用',
  ...
  const REQUIRED_DOCS = [
    ...
    { id: 'proof_address', label: t.proofAddress, desc: t.proofAddressDesc },
    { id: 'cv', label: t.cv, desc: t.cvDesc },
    { id: 'bank_reference', label: t.bankRef, desc: t.bankRefDesc },
  ];
  ...
  const handleDelete = async (docId: string, filePath: string) => {
    ...
    try {
      const res = await fetch('/api/astana/delete-document', {
        method: 'POST',
        ...
        <div className="bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden flex flex-col lg:col-span-2">
            ...
            <div className="p-8 space-y-8">
                            ...
                            <div className="space-y-2">
                                {[1, 2, 3].map((n) => (
                                  <div key={n} className="flex items-center gap-2">
                                      <span className="text-slate-400 text-sm w-4">{n}.</span>
                                      <input
                                        type="text"
                                        name={`activities_${n}`}
                                        value={formData[`activities_${n}`] || ''}
                        ...
                        {[1, 2, 3].map((n) => (
                          <div key={n} className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                value={formData[`company_name_${n}`] || ''}
                                onChange={handleInputChange}
                                className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm"
                              />
                          </div>
                ...
                <section>
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t.legalForm}:</label>
                            <input
                              type="text"
                              name="legal_form"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">{t.shareCapital}:</label>
                                <input
                                  type="text"
                                  name="share_capital"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">{t.directors}:</label>
                                <input
                                  type="text"
                                  name="directors"
                ...
                {/* 4. Shareholder / Director Details */}
                <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                        <div>
                            ...
                            <input type="text" name="full_name" value={formData.full_name || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t.position}:</label>
                            <input type="text" name="position" value={formData.position || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t.passport}:</label>
                            <input type="text" name="passport" value={formData.passport || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Email:</label>
                            <input type="email" name="email" value={formData.email || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t.contactNumber}:</label>
                            <input type="text" name="contact_number" value={formData.contact_number || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t.country}:</label>
                            <input type="text" name="country" value={formData.country || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t.address}:</label>
                            <input type="text" name="address" value={formData.address || ''} onChange={handleInputChange} className="block w-full border-b border-slate-300 focus:border-indigo-500 focus:outline-none px-2 py-1 bg-transparent sm:text-sm" />
                        </div>
                    </div>
                </section>
